...
2. リクエストが送られると、`routes.rb`で割り当てられているURL/コントローラに基づいて、リクエストに対応するアクションを実行します。
（ここでは、usersコントローラのindexアクションを呼び出すような記述になっています。）
...
5. ビューテンプレートのレンダリング結果をHTTPレスポンスでユーザーに返します。
